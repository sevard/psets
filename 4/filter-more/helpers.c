...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        for (int j = 0; j < width; j++)
        {
            ...
            gx_triple = calculate_sobel(y_axis, x_axis, height, width, image);
    ...
    {
        RGBTRIPLE gy_triple;
        update_axis_values(i, height, x_axis);
        for (int j = 0; j < height; j++)
        {
            update_axis_values(j, width, y_axis);
            gy_triple = calculate_sobel(y_axis, x_axis, height, width, image);
            images[j][i];
        }
    }
